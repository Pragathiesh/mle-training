...
def test_ingest():
    ...
    os.system(
        f"python src/ingest_data.py --datapath {datapath}"
    )
...
def test_train():
    ...
    os.system(
        f"python src/train.py --inputpath {dataset} --outputpath {models}"
    )
    print(f"{path}{models}/{model_names[0]}/model.pkl")
    assert os.path.isfile(f"{path}{models}{model_names[0]}")
